...
enum
{
	...
	PGM_PKT_STATE_BACK_OFF    = 1, /* PGM protocol recovery states */
	PGM_PKT_STATE_WAIT_NCF    = 2,
	PGM_PKT_STATE_WAIT_DATA   = 3,
	PGM_PKT_STATE_HAVE_DATA   = 4, /* data received waiting to commit to application layer */
	PGM_PKT_STATE_HAVE_PARITY = 5, /* contains parity information not original data */
	PGM_PKT_STATE_COMMIT_DATA = 6, /* commited data waiting for purging */
	PGM_PKT_STATE_LOST_DATA   = 7  /* if recovery fails, but packet has not yet been commited */
};
...
/* window context counters */
	...
	uint32_t		committed_count;	/* but still in window */
        uint32_t		lead, trail;
        uint32_t		rxw_trail, rxw_lead, rxw_trail_init;
	uint32_t		commit_lead;

        uint16_t		max_tpdu;               /* maximum packet size */
        uint32_t		is_constrained:1;
        uint32_t		is_defined:1;
	uint32_t		has_event:1;		/* edge triggered */
	uint32_t		is_fec_available:1;
	pgm_rs_t		rs;
...
PGM_GNUC_INTERNAL void pgm_rxw_purge_lost (pgm_rxw_t*const);
...
static inline uint32_t pgm_rxw_commit_length ( const pgm_rxw_t* const ) PGM_GNUC_WARN_UNUSED_RESULT;
static inline bool pgm_rxw_commit_is_empty ( const pgm_rxw_t* const ) PGM_GNUC_WARN_UNUSED_RESULT;
static inline uint32_t pgm_rxw_incoming_length ( const pgm_rxw_t* const ) PGM_GNUC_WARN_UNUSED_RESULT;
static inline bool pgm_rxw_incoming_is_empty ( const pgm_rxw_t* const ) PGM_GNUC_WARN_UNUSED_RESULT;
static inline uint32_t pgm_rxw_tg_sqn( pgm_rxw_t* const window, const uint32_t sequence ) PGM_GNUC_WARN_UNUSED_RESULT;
static inline uint32_t pgm_rxw_pkt_sqn( pgm_rxw_t* const window, const uint32_t sequence ) PGM_GNUC_WARN_UNUSED_RESULT;
static inline bool pgm_rxw_is_first_of_tg_sqn( pgm_rxw_t* const window, const uint32_t sequence ) PGM_GNUC_WARN_UNUSED_RESULT;
static inline bool pgm_rxw_is_last_of_tg_sqn ( pgm_rxw_t* const window, const uint32_t sequence ) PGM_GNUC_WARN_UNUSED_RESULT;
#endif
...
static inline uint32_t pgm_rxw_commit_length ( const pgm_rxw_t* const  window) {
        return window->commit_lead - window->trail;
}
static inline bool pgm_rxw_commit_is_empty ( const pgm_rxw_t* const  window) {
        return (pgm_rxw_commit_length (window) == 0);
}
static inline uint32_t pgm_rxw_incoming_length ( const pgm_rxw_t* const  window) {
        return ( 1 + window->lead ) - window->commit_lead;
}
static inline bool pgm_rxw_incoming_is_empty ( const pgm_rxw_t* const  window) {
        return (pgm_rxw_incoming_length (window) == 0);
}
static inline uint32_t pgm_rxw_tg_sqn( pgm_rxw_t* const window, const uint32_t sequence ) {
        const uint32_t tg_sqn_mask = 0xffffffff << window->tg_sqn_shift;
        return sequence & tg_sqn_mask;
}
static inline uint32_t pgm_rxw_pkt_sqn( pgm_rxw_t* const window, const uint32_t sequence ) {
        const uint32_t tg_sqn_mask = 0xffffffff << window->tg_sqn_shift;
        return sequence & ~tg_sqn_mask;
}
static inline bool pgm_rxw_is_first_of_tg_sqn( pgm_rxw_t* const window, const uint32_t sequence ) {
        return pgm_rxw_pkt_sqn (window, sequence) == 0;
}
static inline bool pgm_rxw_is_last_of_tg_sqn ( pgm_rxw_t* const window, const uint32_t sequence ) {
